...
interface SwapLimitSettingsProps {
  fromAsset?: Asset
  toAsset?: Asset
}

export default function SwapLimitSettings(
  props: SwapLimitSettingsProps
): ReactElement {
  ...
  useEffect(() => {
    if (props.fromAsset) {
      dispatch(fetchSwapPrices(props.fromAsset))
    }
  }, [dispatch])
    ...
    <>
      ...
      >
        <div className="settings_wrap">
          ...
          <div className="row">
            <SharedButton
              type={swap.expiration === "1h" ? "primary" : "secondary"}
              size="small"
            ...
            <SharedButton
              type={swap.expiration === "2h" ? "primary" : "secondary"}
              size="small"
            ...
            <SharedButton
              type={swap.expiration === "1d" ? "primary" : "secondary"}
              size="small"
            ...
            <SharedButton
              type={swap.expiration === "1w" ? "primary" : "secondary"}
              size="small"
      ...
      <div className="labels_wrap standard_width">
        <span className="label">
          {`Transaction Expiry`}
          <div className="info">{swap.expiration}</div>
        </span>
        {/* <span className="label">
          KeeperDAO's Keeper network will execute your order as soon as it finds
          sufficient liquidity, and if it finds MEV, you'll receive ROOK
        </span> */}
      </div>
      <style jsx>
        {`
